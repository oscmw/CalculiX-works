...
# ------------------------------
def safe_surface(tags, name):
    valid = [t for t in tags if t in surface_tags]
    if valid:
        gmsh.model.addPhysicalGroup(2, valid, name=name)
def safe_curve(tags, name):
    valid = [t for t in tags if t in curve_tags]
    if valid:
        gmsh.model.addPhysicalGroup(1, valid, name=name)

safe_surface([22], "skir_sless")
safe_surface([23], "skir_carbo_mid")
safe_surface([17], "skir_carbo_und")

safe_surface(range(1,9), "ovalop1_surf")
safe_surface(range(9,17), "ovalop2_surf")
safe_surface(range(24,28), "opening2_surf")
safe_surface(range(18,22), "manhole_surf")
#safe_surface(range(28,32), "base_plate")

safe_curve([84,89], "base_plate_center_YFIX")
safe_curve([54,57], "skir_sless_top_LOAD")
